...
def render_sop(
    ...
    if exit_pl:
        ...
        e1.metric("初始止蚀", f"{exit_pl.stop_initial:.2f}" if exit_pl.stop_initial else "—")
        e2.metric("T1 减半", f"{exit_pl.t1_price:.2f}" if exit_pl.t1_price else "—")
        e3.metric("T1后止蚀", f"{exit_pl.stop_after_t1:.2f}" if exit_pl.stop_after_t1 else "—")
    ...
    if slip is not None:
        ...
        s1.metric("纸面 R:R", f"{slip.rr_paper:.2f}" if slip.rr_paper is not None else "—")
